...
(async () => {
  if (worker.isMainThread) {
    let latest = await generation(45, 5, 100);
    let average = 0;
    ...
    for (let i = 0; i < 100; i++) {
      latest = await generation(45, 5, 100, 10, latest.slice(0, 50), i == 99);
      average = 0;
      latest.forEach(a => average += a.game.score);
      average /= latest.length;
    ...
    for (let i = 0; i < 3600; i++) {
      ...
      const output = generate([agent.mass[0].position.x, agent.mass[0].position.y, agent.mass[0].velocity.x, agent.mass[0].velocity.y, agent.mass[0].angle].concat(asteroids), worker.workerData.layers);

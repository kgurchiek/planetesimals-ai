...
function generate(inputs, weights, biases) {
  const layers = [new Layer({ neurons: inputs.map(a => ({ value: a }))}, weights[0], biases[0])];
...
async function generation(inputs, layers, neurons, outputs, agentCount, winners, mutators, record = false) {
  ...
  function updateQueue() {
    while (threadQueue.length > 0 && threads < maxThreads) {
      const thread = new worker.Worker('./planetesimals', { workerData: threadQueue[0]});
      thread.on('message', (message) => {
        thread.terminate();
        console.log(agents.push(message));
        threads--;
        updateQueue();
      });
      threads++;
  ...
  if (winners == null) {
    for (let i = 0; i < agentCount; i++) {
      ...
      workerData.weights = new Array(layers).fill().map(layer => layer = new Array(neurons).fill().map(neuron => neuron = new Array(neurons).fill().map(weight => weight = Math.random())));
      workerData.weights[0] = new Array(neurons).fill().map(neuron => neuron = new Array(inputs).fill().map(weight => weight = Math.random()));
      workerData.weights.push(new Array(outputs).fill().map(neuron => neuron = new Array(neurons).fill().map(weight => weight = Math.random())));
      workerData.biases = new Array(layers).fill().map(layer => layer = new Array(neurons).fill(0));
      workerData.biases.push(new Array(outputs).fill(0));
      threadQueue.push(workerData);
    }
    updateQueue();
  } else {
    for (let i = 0; i < winners; i++) threadQueue.push({ record, weights: winners[i].weights, biases: winners[i].biases });
    updateQueue();
    for (let i = 0; i < 2; i++) {
  ...
  //for (let i = 0; i < agents.length; i++) agents[i].score += agents[i].game.startingMassValue - agents[i].game.currentMass;
...
(async () => {
  if (worker.isMainThread) {
    let winner = await generation(42, 2, 20, 5, 100);
    console.log('Generaton 1:', winner.map(a => ({ score: a.game.score, level: a.game.level })));
    for (let i = 0; i < 1; i++) {
      winner = await generation(42, 2, 20, 5, 100, winner.slice(0, 10), winner.slice(0, 50), i == 99);
      console.log(`Generation ${i + 2}:`, winner.map(a => ({ score: a.game.score, level: a.game.level })));
    }
  } else {
    ...
    for (let i = 0; i < 3600; i++) {
      ...
      const output = generate([agent.mass[0].position.x, agent.mass[0].position.y].concat(asteroids), worker.workerData.weights, worker.workerData.biases);
      ...
      agent.keys[40] = output.neurons[3].value > 0.5; // down
      agent.keys[32] = output.neurons[4].value > 0.5; // space
      agent.cycle();
    }

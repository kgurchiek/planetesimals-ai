...
module.exports = (record) => {
  ...
  function levelScaling() {
    ...
    game.totalMass = 0; //3 + game.level * 1; //how many masses to spawn at start of level
  ...
  function addPlayer() {
    ...
    mass[0] = Matter.Bodies.fromVertices(1000 + Math.cos(spawnAngle) * 1200, 900 + Math.sin(spawnAngle) * 1200, arrow, {
    // mass[0] = Matter.Bodies.fromVertices(Math.random() * game.width, Math.random() * game.height, arrow, {
      ...
      rotationLimit: 0.05, //0.05, //max acceleration for player in radians/cycle
      angularFriction: 0.98, //0.98, // 1 = no friction,  0.9 = high friction
      durability: 1,
      fireCD: 0,
